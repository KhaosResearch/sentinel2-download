...
if __name__ == "__main__":
    ...
    for tile in tiles:
        ...
        try:
            ...
            print(
                f"Error looking for new data for {tile} from {look_from} to {now}: {e}"
            )

    if now.day == 1:
        init_date = (now - timedelta(days=1)).replace(day=1)
        init_date = datetime(
            init_date.year, init_date.month, 1
        )  # To fix the hour to 00:00:00
        ...
        for tile in tiles:
            ...
            try:
                ...
                print(
                    f"Error creating composite for {tile} from {init_date} to {end_date}: {e}"
                )
