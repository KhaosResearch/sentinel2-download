...
def calculate_no_data(folder: str, is_geojson: bool = False) -> float:
    ...
    """
    if is_geojson:
        tif_options = list(Path(folder).glob("*B0*.tif")) or list(
            Path(folder).glob("*.tif")
        )
...
def load_geojson_shapes(geojson_path: str) -> list[dict]:
    ...
    if isinstance(geojson_obj, dict) and geojson_obj.get("features"):
        ...
        raise ValueError(
            f"GeoJSON file does not contain a valid geometry: {geojson_path}"
        )

    if not shapes:
        raise ValueError(
            f"GeoJSON file contains no valid geometry shapes: {geojson_path}"
        )
...
def mask_jp2_to_geotiff(
    ...
    try:
        ...
        logger.warning(
            f"Failed to mask JP2 to GeoTIFF due to pipeline execution error: {e}"
        )
        return False
...
def _resolve_safe_blob_prefix(blob_name: str) -> tuple[str, str]:
    ...
    if safe_index is None:
        raise ValueError(
            f"Could not resolve SAFE product folder from blob: {blob_name}"
        )
...
def get_google_blobs_metadata(
    product_title: str, gcloud_bucket_name: str, storage_client: Any
    ...
    if len(blobs) == 0:
        ...
        logger.warning(
            f"Path in cloud storage should be {source_blob_name}, but it was not found."
        )
        logger.warning(
            "Trying to find alternative name for the same product (different product discriminator)"
        )
        ...
        if len(blobs) == 0:
            logger.debug(
                "Alternative name not found in Google Cloud. Skipping product."
            )
            return
        else:
...
def is_product_already_stored(
    product_title: str, mongo_col: MongoConnection, minio_client: MinioConnection
    ...
    try:
        ...
        objects = minio_client.list_objects(
            ...
            prefix=join(minio_dir, product_title),
            recursive=False,
        )
        for _ in objects:
            minio_found = True
        ...
        logger.exception("Product not found in MinIO")
...
def download_blob_data(
    ...
    if is_geojson and geojson_path:
        ...
        local_tif_path = (
            Path(tmp_dir) / product_title / image_name.replace(".jp2", ".tif")
        )
        ...
        local_blob_name = Path(
            blob.name[len(source_blob_name + "/") :]
...
def generate_sentinel_metadata(
    ...
    metadata["datetakeSensingTime"] = datetime.strptime(
        product_title.split("_")[2], "%Y%m%dT%H%M%S"
    )
...
def download_one_google_cloud(
    ...
    blobs_metadata = get_google_blobs_metadata(
        product_title, gcloud_bucket_name, storage_client
    )
    ...
    minio_dir, product_mongo_data, minio_found = is_product_already_stored(
        product_title, mongo_col, minio_client
    )
    ...
    if product_mongo_data and minio_found:
        ...
        for blob in blobs:
            if (
                blob.name.endswith("/")
                or "IMG_DATA" not in blob.name
                or not _matches_required_band(blob.name, required_bands)
            ):  # Ignore folders
                continue
            blob_data = download_blob_data(
                blob,
                product_title,
                tmp_dir,
                source_blob_name,
            ...
            local_images.append(blob_data) if not isinstance(
                blob_data, str
            ) else logger.warning(blob_data)
        ...
        if not local_images or len(local_images) < 1:
            ...
            downloaded_band_names = {
                "_".join(
                    os.path.basename(os.path.splitext(image[-1])[0]).split("_")[-2:]
                ).replace(".jp2", "")
        ...
        if required_bands and not set(required_bands).issubset(downloaded_band_names):
            ...
            logger.warning(
                f"Missing required bands for {product_title}: {missing_bands}. Skipping product."
            )
            return
        ...
        try:
            ...
            logger.exception("Product not uploaded to MinIO")
            return
