...
def _build_multi_tile_product_title(product_titles: list[str]) -> str:
    ...
    tile_ids = sorted(
        {title.split("_T")[1][0:5] for title in sorted_titles if "_T" in title}
    )
    if len(tile_ids) <= 1:
        return base_title
...
def _download_band_blobs_for_group(
    ...
    for product_title in product_titles:
        blobs_metadata = get_google_blobs_metadata(
            product_title, bucket_name, storage_client
        )
...
def download_multi_tile_geojson(
    ...
    if not tmp_dir or not gcloud_bucket_name:
        raise EnvironmentError(
            "TMP_DIR and GOOGLE_CLOUD_BUCKET_NAME must be set for multi-tile GeoJSON downloads."
        )

    if required_bands is None:
        raise ValueError(
            "required_bands is required for multi-tile GeoJSON processing."
        )
    ...
    for date_str, products in date_groups.items():
        product_titles = [
            _normalize_product_title(item.get("Name") or item.get("name"))
        ...
        band_blobs = _download_band_blobs_for_group(
            product_titles, required_bands, storage_client, gcloud_bucket_name
        )
        ...
        for band, blobs in band_blobs.items():
            ...
            output_path = process_multi_tile_geojson(
                blobs, geojson_path, local_band_path, mosaic_band_path
            )
            ...
            _upload_to_minio(
                minio_client,
                output_path,
                join(raw_prefix, output_path.name),
                "image/tif",
            )
            _upload_to_minio(
                minio_client,
                mosaic_band_path,
                join(intermediate_prefix, mosaic_band_path.name),
                "image/tif",
            )
            uploaded_count += 1
        ...
        if uploaded_count != len(required_bands):
            logger.warning(
                f"Multi-tile GeoJSON date {date_str} did not produce all required bands. Cleaning local state."
            )
            try:
                ...
                logger.warning(
                    f"Unable to clean local files for {product_title}: {exc}"
                )
            continue

        blobs_metadata = get_google_blobs_metadata(
            product_titles[0], gcloud_bucket_name, storage_client
        )
        if blobs_metadata is None:
            logger.warning(
                f"Could not resolve Google Cloud source metadata for {product_titles[0]}. Skipping."
            )
            continue
        ...
        try:
            ...
            logger.warning(
                f"Failed to insert Mongo metadata for {product_title}: {exc}"
            )
        ...
        try:
            ...
            logger.warning(
                f"Unable to clean local multi-tile temp files for {product_title}: {exc}"
            )
