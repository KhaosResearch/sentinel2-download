...
def log_time(
    ...
    with open(file_path, "a") as log_file:
        log_file.write(
            f"Year: {year}, Month: {month}, Tile: {tile}, Function: {function_name}, Time: {execution_time:.2f} seconds\n"
        )


def main_workflow(
    tiles: list, year: int, from_month: int, to_month: int, delete_products: bool = True
) -> None:
    """
    ...
    for month in range(from_month, to_month + 1):
        for tile in tiles:
            try:
                ...
                end_date = (init_date + timedelta(days=31)).replace(
                    day=1
                ...
                download_product_using_sentinel_api(
                    False, True, init_date, end_date, tile_id=tile
                )
                ...
                create_composite_by_tile_and_date(
                    True, False, tile, init_date, end_date, 30, delete_products
                )
                ...
                error_message = (
                    f"Year: {year}, Month: {month}, Tile: {tile}, Error: {str(e)}"
                )
